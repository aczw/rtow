...
std::optional<ScatterResult> DieletricMaterial::scatter(const Ray& incoming_ray,
  ...
  double ri_in_context =
      isect.get_side() == Surface::Front ? 1.0 / refraction_index : refraction_index;

  Vec3 incoming_ray_direction = incoming_ray.get_direction().normalized();
  Vec3 refracted_ray =
      Vec3::refract_about_normal(incoming_ray_direction, isect.get_normal(), ri_in_context);
