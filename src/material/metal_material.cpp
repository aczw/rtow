...
std::optional<ScatterResult> MetalMaterial::scatter(const Ray& incoming_ray,
  ...
  // For particular fuzz values, the resulting ray might travel "through" the geometry. In this
  // case, simply act as if the scattered ray has been absorbed
  if (Vec3::dot(reflected_direction, isect_normal) < 0.0)
    return std::nullopt;
